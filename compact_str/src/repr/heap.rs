...
impl Cap {
    ...
    fn new(l: usize) -> Result<Self, TooLong> {
        ...
        if l_bytes[mem::size_of::<usize>() - 1] == 0 {
            Ok(Cap(l_bytes[..mem::size_of::<usize>() - 1].try_into().unwrap()))
        } else {
            Err(TooLong)
        }
    }
...
impl ThinHeapBuffer {
    ...
    pub(crate) fn new(text: &str) -> ThinHeapBuffer {
        dbg!(text.len());
        todo!()
    }

    pub(crate) fn with_capacity(capacity: usize) -> ThinHeapBuffer {
        todo!()
    }

    pub(crate) fn with_additional(as_str: &str, additional: usize) -> ThinHeapBuffer {
        todo!()
    }

    pub(crate) fn realloc(&mut self, new_capacity: usize) {
        todo!()
    }
    ...
    pub(crate) fn as_mut_buf(&self) -> &mut [MaybeUninit<u8>] {
        todo!()
    }
}
...
impl HeapBuffer {
    ...
    /// Sets the length of the content for this [`HeapBuffer`]
    ///
    /// # SAFETY:
    ...
    pub(crate) unsafe fn as_mut_buf(&mut self) -> &mut [MaybeUninit<u8>] {
        // SAFETY: this type's invariant guarantees that self.ptr is allocated for at least self.cap
        // the caller must guarantee that they only write valid UTF-8
        slice::from_raw_parts_mut(self.ptr.as_ptr().cast(), self.cap.into())
    }

    pub(crate) fn as_slice(&self) -> &[u8] {
        // SAFETY: this type's invariant guarantees that self.ptr is allocated and initialized for at least self.len
...
mod test {
    ...
    use super::{HeapBuffer, TooLong, MIN_HEAP_SIZE};
    ...
    fn test_realloc(buf: &[u8], realloc: usize, result: Result<usize, usize>) {
        ...
        let expected_res = result.map_err(|_| TooLong);
    ...
    fn test_realloc_shrink(
        ...
        let expected_res = exp_result.map_err(|_| TooLong);
